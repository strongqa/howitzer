...
RSpec.describe Howitzer::Web::SectionDsl do
  ...
  describe 'dynamic_methods' do
    context 'when section with single argument' do
      ...
      describe '#name_section' do
        ...
        before { expect(session).to receive(:find).with(:xpath, './/div') { capybara_element } }
      ...
      describe '#name_sections' do
        ...
        before do
          expect(session).to receive(:all).with(:xpath, './/div') { [capybara_element1, capybara_element2] }
        end
      ...
      describe '#has_name_section?' do
        ...
        before { expect(session).to receive(:has_selector?).with(:xpath, './/div') { true } }
        it { is_expected.to eq(true) }
      end
      describe '#has_no_name_element?' do
        subject { web_page_object.send(:has_no_foo_section?) }
        before { expect(session).to receive(:has_no_selector?).with(:xpath, './/div') { true } }
        it { is_expected.to eq(true) }
      end
    end
  end
